...
def createDatasets() {
    def properties = BuildProperties.getInstance()
	def srcOptions = "cyl space(1,1) lrecl(80) dsorg(PO) recfm(F,B) dsntype(library) msg(1) vol(VPWRKD)"
	def loadOptions = "cyl space(1,1) dsorg(PO) recfm(U) blksize(32760) dsntype(library) msg(1) vol(VPWRKD)"
...
def getDefaultDependencyResolver(String file) {
    def properties = BuildProperties.getInstance()
	def path = new DependencyPath().sourceDir(properties.sourceDir).directory("MFTR33/COPYBOOK")
...
def getDefaultImpactResolver(String file) {
	...
	// Add rule to assocuate changes to copybooks with the need to recompile programs that reference them.
   	def cpypath = new DependencyPath().collection(properties.collection).sourceDir(properties.sourceDir).directory("MFTR33/COPYBOOK")
	...
	// Add rule to associate changes to BMS maps with need to recompile programs that use the generated copybook.
   	def bmspath = new DependencyPath().collection(properties.collection).sourceDir(properties.sourceDir).directory("MFTR33/BMS")
	...
	// Add rule to associate changes to COBOL source with the need to linkedit .lnk files that reference them.
   	def cobpath = new DependencyPath().collection(properties.collection).sourceDir(properties.sourceDir).directory("MFTR33/COBOL")
